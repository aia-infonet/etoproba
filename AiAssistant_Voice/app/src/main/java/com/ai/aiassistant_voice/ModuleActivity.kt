 ...
 *
 * Версия 3.7: исправлен race condition при синхронизации с TTS сервера.
 * - zakaz_prompt с has_tts=true больше не запускает микрофон немедленно
 * - tts_ended корректно перезапускает zakaz-слушание
 */
class ModuleActivity : AppCompatActivity() {
    ...
    // === Поля для синхронизации с TTS сервера ===
    private var ttsIsPlaying = false
    private var pendingAutoListen = false
    private var wasRecordingBeforeTts = false
    // =============================================
    ...
    override fun onPause() {
        super.onPause()
        Log.d(TAG, "onPause, currentState=$currentState, ttsIsPlaying=$ttsIsPlaying")
        when (currentState) {
            "recording" -> {
    ...
    private fun onCommandButtonClick(commandName: String, label: String, button: Button) {
        Log.d(TAG, "Нажата кнопка команды=$commandName, currentState=$currentState, ttsIsPlaying=$ttsIsPlaying")
        ...
        wsManager.onMessage = { data ->
            runOnUiThread {
                ...
                when (data["type"] as? String) {
                    "converted" -> {
                        val original = data["original"] as? String ?: ""
                        val converted = data["converted"] as? String ?: ""
                        SharedLog.append("Распознано: $original")
                        SharedLog.append("Преобразовано: $converted")
                    }
                    "tts_started" -> {
                        ttsIsPlaying = true
                        val ttsText = data["text"] as? String ?: ""
                        SharedLog.append("TTS сервера началось: $ttsText")
                        Log.d(TAG, "tts_started, currentState=$currentState")

                        // Если микрофон активен — приостанавливаем слушание
                        if (currentState == "recording") {
                            wasRecordingBeforeTts = true
                            voskManager.stopListening()
                            SharedLog.append("Микрофон приостановлен (TTS сервера)")
                            tvCommandStatus.text = "Сервер говорит..."
                        }
                    }
                    "tts_ended" -> {
                        ttsIsPlaying = false
                        val ttsText = data["text"] as? String ?: ""
                        SharedLog.append("TTS сервера окончено: $ttsText")
                        Log.d(TAG, "tts_ended, pendingAutoListen=$pendingAutoListen, wasRecordingBeforeTts=$wasRecordingBeforeTts")

                        // === ИСПРАВЛЕНИЕ: корректная обработка tts_ended ===
                        if (pendingAutoListen) {
                            // Отложенное авто-слушание zakaz
                            pendingAutoListen = false
                            startAutoListeningForZakaz()
                        } else if (wasRecordingBeforeTts) {
                            wasRecordingBeforeTts = false
                            if (activeCommandName == "zakaz") {
                                // Для zakaz перезапускаем авто-слушание
                                startAutoListeningForZakaz()
                            } else if (activeCommandName.isNotEmpty() && activeCommandName !in instantCommands) {
                                // Для ручных команд возобновляем запись
                                voskManager.startListening()
                                currentState = "recording"
                                tvCommandStatus.text = "Слушаю..."
                                SharedLog.append("Микрофон возобновлён")
                            }
                        }
                        // ==================================================
                    }
                    ...
                    "zakaz_prompt" -> {
                        ...
                        val hasTts = data["has_tts"] as? Boolean ?: false
                        ...
                        if (App.instance.isVoskReady) {
                            if (hasTts) {
                                // === ИСПРАВЛЕНИЕ: не запускаем микрофон немедленно ===
                                pendingAutoListen = true
                                SharedLog.append("Ожидание окончания TTS сервера...")
                                // Убран вызов startAutoListeningForZakaz() здесь
                                // ==================================================
                            } else {
                                startAutoListeningForZakaz()
                            }
    ...
    private fun startAutoListeningForZakaz() {
        if (ttsIsPlaying) {
            pendingAutoListen = true
            SharedLog.append("Ожидание окончания TTS перед началом слушания...")
            return
        }
        pendingAutoListen = false
        currentState = "recording"
        tvCommandStatus.text = "Слушаю..."
        SharedLog.append("Предупреждение! Произносите нули отдельным словом. Не говорите двадцать (если ровно 20), двести (если ровно 200) и т.д.")
    ...
    private fun startCommand(commandName: String, label: String, button: Button) {
        ...
        // Если сервер говорит — не начинаем запись
        if (ttsIsPlaying) {
            Toast.makeText(this, "Сервер озвучивает текст, подождите...", Toast.LENGTH_SHORT).show()
            return
        }

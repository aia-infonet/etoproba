...
class ServerGUI:
    def __init__(self, root: tk.Tk):
        ...
        self.log_text = scrolledtext.ScrolledText(
            self.frame_log,
            state='normal',           # ← разрешаем выделение
            wrap=tk.WORD,
            font=("Consolas", 10),
            bg="#1e1e1e",
            fg="#d4d4d4",
            insertbackground="white",
            insertwidth=0               # скрываем курсор вставки
        )
        self.log_text.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)

        # Защита от редактирования: блокируем все клавиши кроме Ctrl+C, Ctrl+A
        ...
        self.log_text.bind('<Button-3>', self._show_context_menu)  # правый клик
    ...
    def _on_log_key(self, event):
        """Блокирует ввод текста, но разрешает Ctrl+C и Ctrl+A."""
        # event.state & 0x4 == Control нажат
        if event.state & 0x4 and event.keysym.lower() in ('c', 'a'):
            return None
        return 'break'

    def _show_context_menu(self, event):
        """Контекстное меню: Копировать / Выделить всё."""
    ...
    def log(self, message: str):
        ...
        # state остается 'normal' для копирования
